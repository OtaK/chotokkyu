...
/**
 * Create chotokkyu application at `path`.
 *
 * @param {String} path
 * @api private
 */
exports = module.exports = function create(path) {
  console.log('creating chotokkyu application at : ' + path);
...
var pagesController = [
    'var chotokkyu = require(\'chotokkyu\')'
  , '  , Controller = chotokkyu.Controller;'
  , ''
  , 'var pagesController = new Controller();'
  , ''
  , 'pagesController.main = function() {'
  , '  this.title = \'chotokkyu\';'
  , '  this.render();'
  , '}'
  , ''
...
var mainTemplate = [
  ...
  , '  <body>'
  , '    <h1><%= title %></h1>'
  , '    <p>Welcome aboard! Visit the <a href="https://github.com/viadeo/chotokkyu">GitHub project page</a> for details.</p>'
  , '  </body>'
  , '</html>'
].join(eol);
...
var routesTemplate = [
    '// Draw routes.  chotokkyu\'s router provides expressive syntax for drawing'
  , '// routes, including support for resourceful routes, namespaces, and nesting.'
  , '// MVC routes can be mapped to controllers using convenient'
  , '// `controller#action` shorthand.  Standard middleware in the form of'
  , '// `function(req, res, next)` is also fully supported.  Consult the chotokkyu'
  , '// Guide on [routing](http://chotokkyujs.org/guide/routing.html) for additional'
  , '// information.'
...
var allEnvironments = [
  ...
  , '  // of chotokkyu.'
  , '  if (this.version !== require(\'chotokkyu\').version) {'
  , '    console.warn(\'version mismatch between local (%s) and global (%s) chotokkyu module\', require(\'chotokkyu\').version, this.version);'
  , '  }'
  , '}'
  , ''
].join(eol);
...
var viewsInitializer = [
  ...
  , ''
  , '  // Override default template extension.  By default, chotokkyu finds'
...
var serverJS = [
  ...
  , 'var app = new chotokkyu.Application();'
  , 'app.phase(chotokkyu.boot.controllers(__dirname + \'/app/controllers\'));'
  ...
  , 'app.phase(chotokkyu.boot.routes(__dirname + \'/config/routes\'));'
  , 'app.phase(chotokkyu.boot.httpServer(3000, \'0.0.0.0\'));'
  , ''
...
var packageJSON = [
  ...
  , '  "private": true,'
  , '  "dependencies": {'
  , '    "chotokkyu": "viadeo/chotokkyu#v0.6.3",'
  , '    "bootable": "0.2.x",'
