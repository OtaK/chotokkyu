...
var router = require('actionrouter')
  ...
  , deprecate = require('depd')('chotokkyu')
...
Controller.prototype.render = function (template, options, fn) {
  ...
  // delete option keys consumed by chotokkyu
 ...
 *     })
 *
 * By default chotokkyu passes an `Error` with a `.status` of 406 to
...
Controller.prototype.respond =
  Controller.prototype.respondWith = function (obj) {
    ...
    format = format ? mime.getType(format) : undefined;
...
Controller.prototype.invoke = function (controller, action) {
  ...
  dispatch(this.__app, controller, action)(this.__req, this.__res);
};
...
/**
 * Invoked when controller is initialized by chotokkyu.
 *
 * This function is called immediately after chotokkyu instantiates the
...
Controller.prototype._invoke = function (action) {
  ...
  // Assign chotokkyu properties to request.  These properties are used by
  // helper functions, such as urlFor(), to return results based on the context
  // of the request and response.
  this.__req._chotokkyu = {};
  this.__req._chotokkyu.app = this.__app;
  this.__req._chotokkyu.controller = this.__id;
  this.__req._chotokkyu.action = this.__action;
