...
/**
 * `chotokkyu` constructor.
 *
 * A default `chotokkyu` singleton is exported via the module.  Applications
 ...
 *
 * chotokkyu provides support for content negotiation, allowing a single route
 ...
 *
 * By default, chotokkyu looks for template files using a convention of
 ...
 *
 * To facilitate mapping models to controllers, chotokkyu introspects models
 ...
 * should be registered with chotokkyu to provide the necessary introspection
 * logic.
 *
 * @param {Module} store
 * @api public
 ...
 * a specific controller and action in this chotokkyu appliction.  This is
 ...
 *
 *     this.use(chotokkyu.invoke());
 *
 * If you are mounting multiple chotokkyu applications in a single server, a
 ...
 *
 * Controller's are auto-required by chotokkyu.  The value exported by the
 ...
 *
 * chotokkyu uses an MVC-style router, where routes map from a URL pattern to
...
/**
 * Boot `chotokkyu` application.
 *
 * chotokkyu builds on Express, providing a set of conventions for how to
 * organize code and resources on the file system as well as an MVC architecture
 * for structuring code.
 *
 * When booting a chotokkyu application, the file system conventions are used
